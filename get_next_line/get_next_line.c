...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/05 11:55:46 by llombard          #+#    #+#             */
/*   Updated: 2022/11/16 12:56:45 by llombard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (NULL);
	strtemp = get_new_line(strtemp, fd);
	str = get_return_line(strtemp);
	strtemp = get_extra_line(strtemp);
	return (str);
}
